...
// `own` here means "label the sentence as own vessel (!AIVDO)".
function routing (transceiver, mmsi, ctx) {
  ...
  // Some services ignore !AIVDO, so the own-vessel sentence can go out as !AIVDM.
  return { channel: channelOf(transceiver), own: own && ctx.ownAsAivdm !== true }
}
